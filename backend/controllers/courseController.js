...
export const editCourse = async (req,res) => {
    try {
        ...
        if(level !== undefined){
            ...
            else $set.level = level
        }

        if(price !== undefined){
            if(price === "") $unset.price = 1
            else $set.price = price
        }

        if(isPublished !== undefined && isPublished !== ""){
            $set.isPublished = isPublished === true || isPublished === "true"
        }
        ...
        course = await Course.findByIdAndUpdate(courseId , update , {new:true, runValidators:true})
        return res.status(201).json(course)
    } catch (error) {
...
export const editLecture = async (req,res) => {
    try {
        ...
        const {isPreviewFree , lectureTitle, videoUrl: videoUrlFromBody} = req.body
        ...
        } else if (typeof videoUrlFromBody === "string" && videoUrlFromBody.trim()) {
            lecture.videoUrl = videoUrlFromBody.trim()
        }
        if(lectureTitle){
